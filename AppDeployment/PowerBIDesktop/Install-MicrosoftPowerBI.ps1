...
.OUTPUTS
    Log file saved to: C:\ProgramData\_MEM\Install-MicrosoftPowerBI.log

.NOTES
    File Name : Install-MicrosoftPowerBI.ps1  
...
.EXAMPLE
    Installs MicrosoftPowerBI:
    winget install --id Microsoft.PowerBI -e --silent
#>
...
& $winget_exe install --exact --id $ProgramName --silent --accept-package-agreements --accept-source-agreements --scope=machine $param
  $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Microsoft PowerBI Desktop").InstalledVersion
