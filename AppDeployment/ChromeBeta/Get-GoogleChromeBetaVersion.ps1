...
.NOTES
    File Name : Get-GoogleChromeBetaVersion.ps1  
    Author    : Paul Gosling, Hexagon Technology Services  
    Created   : 2025-05-14  
    Requires  : WinGet, Windows 10/11
#>

$productName = "GoogleChromeBeta"   #------------------------------------------# Application name or function for logfile  
$programName = "Google.Chrome.Beta"   #----------------------------------------# WinGet application ID 
...
if (!$winget_exe){
    ...
    $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Google Chrome Beta").InstalledVersion
