...
.NOTES
    File Name : Get-OracleVirtualBoxVersion.ps1  
    Author    : Paul Gosling, Hexagon Technology Consulting  
    Created   : 2025-05-14  
    Requires  : WinGet, Windows 10/11
#>

$productName = "OracleVirtualBox"   #------------------------------------------# Application name or function for logfile  
$programName = "Oracle.VirtualBox"   #-----------------------------------------# WinGet application ID 
...
if (!$winget_exe){
    ...
    $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Oracle VM VirtualBox").InstalledVersion
