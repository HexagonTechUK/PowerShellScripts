...
.NOTES
    File Name : Get-AdobeCreativeCloudVersion.ps1  
    Author    : Paul Gosling, Hexagon Technology Services  
    Created   : 2025-05-14  
    Requires  : WinGet, Windows 10/11
#>

$productName = "AdobeCreativeCloud"   #-------------------------------------------------# Application name or function for logfile  
$programName = "Adobe.CreativeCloud"   #---------------------------------------------------# WinGet application ID 
...
if (!$winget_exe){
    ...
    $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Adobe Creative Cloud").InstalledVersion
