...
.OUTPUTS
    Log file saved to: C:\ProgramData\_MEM\Install-AzureStorageExplorer.log

.NOTES
    File Name : Install-AzureStorageExplorer.ps1  
...
.EXAMPLE
    Installs AzureStorageExplorer:
    winget install --id Microsoft.Azure.StorageExplorer -e --silent
#>

# Variables (complete these):
$deployType  = "Install"   #-------------------------------------------------------# Deployment type: Install, Upgrade, Removal
$productName = "AzureStorageExplorer"   #------------------------------------------# Application name for logfile
...
& $winget_exe install --exact --id $ProgramName --silent --accept-package-agreements --accept-source-agreements --scope=machine $param
  $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Microsoft Azure Storage Explorer").InstalledVersion
