...
$productName = "7zip"   #------------------------------------------------------# Application name or function for logfile  
$programName = "7zip.7zip"   #-------------------------------------------------# WinGet application ID 
...
if (!$winget_exe){
    ...
    $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "7-Zip").InstalledVersion
