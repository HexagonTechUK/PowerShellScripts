...
.OUTPUTS
    Log file saved to: C:\ProgramData\_MEM\Install-MozillaFirefox.log

.NOTES
    File Name : Install-MozillaFirefox.ps1  
...
$productName = "MozillaFirefox"   #------------------------------------------------# Application name for logfile
$programName = "Mozilla.Firefox"   #-----------------------------------------------# Deployment type: Install, Upgrade, Removal
...
& $winget_exe install --exact --id $ProgramName --silent --accept-package-agreements --accept-source-agreements --scope=machine $param
  $wingetPrg_Existing = & $winget_exe list --id $ProgramName --exact --accept-source-agreements
        ...
        $version = (Get-WinGetPackage -Name "Mozilla Firefox (en-US)").InstalledVersion
